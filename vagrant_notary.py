...
class vagrant_notary(ShutItModule):
	...
	def build(self, shutit):
		...
		shutit.send('curl -sSL https://experimental.docker.com/ | sh',note='Install the experimental version of docker')
		shutit.send('curl -L https://github.com/docker/compose/releases/download/1.4.0/docker-compose-`uname -s`-`uname -m` > /usr/local/bin/docker-compose',note='Install docker-compose')
...
ENTRYPOINT ["bash"]''',note='Creating dockerfile for sandbox build')
		...
		shutit.pause_point('new passwords')
		...
		shutit.send('cd /var/lib/registry/docker/registry/v2/blobs/sha256/')
		shutit.send('for f in $(ls); do for f2 in $(ls $f); do echo "Evil data" > /var/lib/registry/docker/registry/v2/blobs/sha256/$f/$f2/data; done; done',note='Placing bad data into the registry!')
		shutit.logout()
		shutit.send('docker images -q -a | xargs docker rmi -f || /bin/true',note='Removing the images we have locally we do not need')
		shutit.login(command='docker exec -ti notarysandbox bash')
		shutit.send('export DOCKER_CONTENT_TRUST=1',note='Enable the docker trust env variable')
		shutit.send('export DOCKER_CONTENT_TRUST_SERVER=https://notaryserver:4443',note='Telling Docker which server we are trusting (normally defaults to the Docker Hub)')
		shutit.send('docker pull sandboxregistry:5000/test/alpine:latest',note='Pulling the image again, it should fail to verify!')
		shutit.logout()
		shutit.logout()
		shutit.logout()
		return True
