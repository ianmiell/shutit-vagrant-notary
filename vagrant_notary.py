...
ENTRYPOINT ["bash"]''',note='Creating dockerfile for sandbox build')
		...
		shutit.send(command='docker run -d --name notarysandbox -v /var/run/docker.sock:/var/run/docker.sock --link notary_notaryserver_1:notaryserver --link sandboxregistry:sandboxregistry notarysandbox',note='Logging into the notary sandbox, linked to the notary server')
